...
def second_step_of_connectivity_repair(list_disjoints_neighbors, graph, coordinates):
    ...
    for i in range(num_terminals):
        for j in range(i+1, num_terminals):
            ...
            pair['dis'] = distance(coordinates[terminals[i]][0], coordinates[terminals[i]][1], coordinates[terminals[j]][0], coordinates[terminals[j]][1])
            pair['inserted']= False
...
def connectivity_repair_heuristic(disjoint_sets, individual, graph, coordinates):
    steiner_tree = second_step_of_connectivity_repair(disjoint_sets, graph, coordinates)
...
def generate_coverage_graph(zones, Rs, Ru):
    ...
    for i in range(len(zones)-1):
        for j in range(i + 1, len(zones)):
            if pm.Elfes_model(zones[i][0], zones[i][1], zones[j][0], zones[j][1], Rs,
                                  Ru):
                if not graph.has_edge(i, j):
                    graph.add_edge(i, j)
