...
max_evaluations_archi5 =100*0.33*nb_zones
disjoint_sets = sp.distinct_connected_components(communication_graph)
if len(disjoint_sets) > 1:
    print("archi5 communication graph not connected nb disjoint sets ", len(disjoint_sets))

print("archi 55 communication graph  connected")
...
for i in range(nb_run):
    ...
    front_mogwo = MOGWO.evolve(max_evaluations_archi5, nb_zones, communication_graph, coordinates, list_target_points,
                               nb_targets, NoGrid_mogow, nRep_mogwo, pop_size_mogwo)
    new_front_mogwo = Optimizer.coverage_cost_front_swarm(front_mogwo)
    Optimizer.save_results('mogwo_archi5-1', str(new_front_mogwo))

    #########################  NSGA-II  #############################
    print("nsga-ii")

    front_nsga = nsga.evolve(max_evaluations_archi5, list_target_points, communication_graph, nb_zones, coordinates,
    ...
    Optimizer.save_results('nsga_archi5-1', str(new_front_nsga))

    #########################  SPEA-II  #############################
    print("spea-ii")

    front_spea = SPEAII.evolve(max_evaluations_archi5, list_target_points, communication_graph, nb_zones, coordinates,
    ...
    Optimizer.save_results('spea_archi5-1', str(new_front_spea))

    #########################  MOPSO #############################
    print("mopso")

    front_pso = MOPSO.evolve(max_evaluations_archi5, nb_zones, communication_graph, coordinates, nb_targets,
                             list_target_points, pop_size_pso, NoGrid_pso, c1, c2, nRep_pso)
    new_front_mopso = Optimizer.coverage_cost_front_swarm(front_pso)
    Optimizer.save_results('mopso_archi5-1', str(new_front_mopso))