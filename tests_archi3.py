...
if len(disjoint_sets) > 1:
    print("archi3 communication graph not connected nb disjoint sets ", len(disjoint_sets))

print("archi 3 communication graph  connected")
max_evaluations_archi3 =100*0.6*nb_zones
...
for i in range(nb_run):
    ...
    #front_mogwo = MOGWO.evolve(max_evaluations_archi3, nb_zones, communication_graph, coordinates, list_target_points,nb_targets, NoGrid_mogow, nRep_mogwo, pop_size_mogwo)
    #new_front_mogwo = Optimizer.coverage_cost_front_swarm(front_mogwo)
    #Optimizer.save_results('mogwo_archi3', str(new_front_mogwo))

    #########################  NSGA-II  #############################
    print("nsga-ii")

    #front_nsga = nsga.evolve(max_evaluations_archi3, list_target_points, communication_graph, nb_zones, coordinates,num_of_tour_particips, tournament_prob, mutation_param_nsga, pop_size_nsga, crossover_nsga)
    #new_front_nsga = Optimizer.coverage_cost_front(front_nsga)
    #Optimizer.save_results('nsga_archi3', str(new_front_nsga))

    #########################  SPEA-II  #############################
    print("spea-ii")

    #front_spea = SPEAII.evolve(max_evaluations_archi3, list_target_points, communication_graph, nb_zones, coordinates, nb_targets, num_of_tour_particips,
    ...
    #Optimizer.save_results('spea_archi3', str(new_front_spea))

    #########################  MOPSO #############################
    print("mopso")

    front_pso = MOPSO.evolve(max_evaluations_archi3, nb_zones, communication_graph, coordinates, nb_targets,
                             list_target_points, pop_size_pso, NoGrid_pso, c1, c2, nRep_pso)
    new_front_mopso = Optimizer.coverage_cost_front_swarm(front_pso)
    Optimizer.save_results('mopso_archi3', str(new_front_mopso))